defmodule Mewtwo.Compression do
  ...
  Args:
    ...
    - token_budget: hard ceiling on the compressed result. Files are dropped
      ...
      until the diff fits.
  """
  def compress(diff_string, token_budget \\ 100_000) do
    ...
    step2 = FileSummarizer.summarize(step1)
    log_stage(:file_summarize, nil, step2)

    # Stage 4: Drop whole files if we are still over budget. Without this a
    # lockfile-heavy PR sails past every guard and the model rejects the
    # request outright.
    {step3, truncation} = Truncator.truncate(step2, token_budget)
...

    {
      ...
      %{
        ...
        stages_applied: [:line_compress, :file_summarize, :truncate]
      }
    }
  end
